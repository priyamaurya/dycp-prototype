...
router.post("/exclusion-start-page", function (req, res) {
  res.redirect("exclusion-penalty-question");
});

router.post("/exclusion-penalty-question", function (req, res) {
  let exclusionDetails = req.session.data.exclusionDetails;
  if (exclusionDetails == "Yes") {
...
router.post("/uk-event", function (req, res) {
  let ukEventDis = req.session.data.ukEventDis;
  if (ukEventDis == "Yes") {
    res.redirect("exclusion-grounds-in-UK");
  } else {
    res.redirect("exclusion-grounds-outside-UK");
  }
});
