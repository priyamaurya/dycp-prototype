...
router.post('/dir-law-register-ni', function (req, res) {
  ...
  if( connectedPersons == 'Equivalent to director or same responsibilities' ){
...
router.post('/connected-question', function (req, res) {
  ...
  - there is currently no screen in the journey that determines
    whether the it's an org or individual
...
// why is this duplicated? note by Z
  ...
  if (postcodeLookup) {
    ...
    if (regex.test(postcodeLookup) === true) {
      ...
      axios.get("https://api.os.uk/search/places/v1/postcode?postcode=" + postcodeLookup + "&key=" + process.env.AXIOS_API_KEY)
        ...
        .catch(error => {
          console.log(error);
          res.redirect('dir-reg-address-uk')
        });
